...
def test_run_acquisition_throttles_runtime_value_prints_by_configured_percent(
    monkeypatch,
    capsys,
) -> None:
    defaults = default_config()
        ...
        def get_all(self) -> list[AcquiredRow]:
            ...
            if self._reads == 1:
                return [
                    AcquiredRow(
                        device_row=np.array(
                            [sample_id, 1, 2, 3, 4, 500 + sample_id, 0],
                            dtype=float,
                        ),
                        source_sample_index=sample_id,
                        capture_time_lsl_s=10.0 + (sample_id / 100.0),
                    )
    ...
    class FakeSessionWriter:
        def __init__(
            ...
            *,
            ...
            del root_dir, app_config, device_sample_width
            self.resolved_config_path = Path("resolved_config.toml")
    ...
    assert captured.out.count("Normalized: 0.5000") == 4
